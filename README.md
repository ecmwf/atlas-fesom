...
**atlas-fesom** is a plugin for [Atlas](https://github.com/ecmwf/atlas) that adds support for FESOM grids and mesh generation.
The plugin registers known FESOM grids and is responsible for retrieving and 
...
<p align="center">
  ...
  <a href="https://github.com/ecmwf/atlas-fesom/releases">
    <img src="https://img.shields.io/github/v/release/ecmwf/atlas-fesom?color=purple&label=Release" alt="Latest Release">
  </a>
</p>
...
Loading of atlas-fesom plugin:
-----------------------------

- If your project explicitly links with the atlas-fesom library, then nothing needs to be done.
  The plugin will be loaded as any explicitly linked library.

- If atlas-fesom is installed in the same install-prefix as the executable or the eckit library, 
...
- Otherwise, two environment variables need to be set, which will help detection and loading
  of plugins:
      ...
      export PLUGINS_MANIFEST_PATH=/path/to/atlas-fesom/share/plugins  # colon separated list
      export LD_LIBRARY_PATH=/path/to/atlas-fesom/lib:$LD_LIBRARY_PATH # use DYLD_LIBRARY_PATH for macOS
    
Verify the plugin is loaded using the `atlas` executable:

    atlas --info
    
should print a "Plugin" section containing "atlas-fesom" with associated version and git sha1.


Using atlas-fesom plugin:
------------------------

Standard tools that link with atlas only should now be able to use Atlas concepts that have been
implemented in the atlas-fesom plugin. 
...
The data files can also be cached beforehand with a provided tool `atlas-fesom-cache`. Use its `--help` argument for more info.

```bash
export ATLAS_FESOM_CACHING=1
export ATLAS_CACHE_PATH=<atlas-cache-dir>
atlas-fesom-cache --grid=<gridname>  # use "all" as gridname to proceed for all available grids
```
...
All fesom data files can also be retrieved and installed as part of the build/install steps.
In order to proceed, configure the build with following CMake arguments:
```bash
-DENABLE_RETRIEVE_FESOM_DATA=ON -DENABLE_INSTALL_FESOM_DATA=ON
```
