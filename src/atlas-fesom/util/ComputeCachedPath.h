        ...
        if ( url.rfind( "http", 0 ) == 0 ) {
            ...
            for ( const auto& known_url : known_urls_ ) {
                if ( url.find( known_url ) == 0 ) {
                    ...
                    return "atlas/grids/fesom" + filepath;
                }
            }
            if ( not is_known_url ) {
                std::string filename = url.substr( url.find_last_of( "/" ), url.size() );
                return "atlas/grids/fesom/unknown" + filename;
            }
    ...
    eckit::PathName operator()( const std::string& url ) const {
        ...
        for ( const auto& known_url : known_urls_ ) {
            if ( url.find( known_url ) == 0 ) {
                is_known_url         = true;
                std::string filepath = url.substr( known_url.size() );
                path                 = Library::instance().cachePath() + "/atlas/grids/fesom" + filepath;
            }
        }
        if ( not is_known_url ) {
            std::string filename = url.substr( url.find_last_of( "/" ), url.size() );
            path                 = Library::instance().cachePath() + "/atlas/grids/fesom/unknown" + filename;
        }
        return path;
    }
