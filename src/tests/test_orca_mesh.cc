...
#include "atlas-fesom/grid/FesomGrid.h"
#include "atlas-fesom/util/PointIJ.h"
...
CASE( "test generate fesom mesh" ) {
    static std::string gridname = "FESOM1";
    static auto grid            = Grid( gridname );

    SECTION( "fesom_generate" ) {
        Log::info() << "grid.footprint() = " << eckit::Bytes( grid.footprint() ) << std::endl;

        auto meshgenerator = MeshGenerator{"fesom"};
        ...
        if ( mesh.footprint() < 25 * 1e6 ) {  // less than 25 Mb
            output::Gmsh{"fesom_2d.msh", Config( "coordinates", "lonlat" )}.write( mesh );
            output::Gmsh{"fesom_3d.msh", Config( "coordinates", "xyz" )}.write( mesh );
        }
...
CASE( "test fesom mesh halo" ) {
    ...
    for ( auto gridname : gridnames ) {
        SECTION( gridname ) {
            ...
            FesomGrid grid = mesh.grid();
            for ( idx_t jnode = 0; jnode < mesh.nodes().size(); ++jnode ) {
                if ( remote_idx( jnode ) < 0 ) {
                    auto p = fesom::PointIJ{ij( jnode, 0 ), ij( jnode, 1 )};
                    fesom::PointIJ master;
