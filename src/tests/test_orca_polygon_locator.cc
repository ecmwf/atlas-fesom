...
CASE( "test fesom polygon locator" ) {
    ...
    for ( auto gridname : gridnames ) {
        SECTION( gridname ) {
            ...
            // fails on FESOM2 because partition polygon is not connected space
            // (FESOM2 grids have a cut out area for the mediterranean)
            idx_t part;
            part = locator({82.0, 10.0});
            ATLAS_DEBUG_VAR( part );
            // would fail on FESOM025 because xy coordinate system doesn't cover
